...
class StatSelectView(discord.ui.View):
    def __init__(self, bot, dino: str):
        ...
        self.select = discord.ui.Select(placeholder="Selecciona la estadística a mutar (+2)", min_values=1, max_values=1, options=options)
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        async with aiosqlite.connect(self.bot.db_name) as db:
            ...
            cursor = await db.execute("SELECT * FROM dinos WHERE especie = ?", (self.dino,))
            row = await cursor.fetchone()
            
            if row:
                old_val = row[stat_selected] or 0
                new_val = old_val + 2
                await db.execute(f"UPDATE dinos SET {stat_selected} = ? WHERE especie = ?", (new_val, self.dino))
                
                # Log the mutation
                logging.getLogger("ArkTribeBot").info(f"MUTATION: {self.dino} {stat_selected} +2")
            else:
                new_val = 2
                await db.execute(f"INSERT INTO dinos (especie, {stat_selected}) VALUES (?, ?)", (self.dino, new_val))
            
            await db.commit()
            
        # Refrescar dashboard
        await interaction.response.edit_message(content=f"✅ Muta registrada: **{self.dino}** (+2 en {stat_selected}). Nuevo valor: {new_val}", view=None)
...
class DinoSelectView(discord.ui.View):
    def __init__(self, bot, dinos):
        ...
        self.select = discord.ui.Select(placeholder="Selecciona el dino", min_values=1, max_values=1, options=options)
        self.select.callback = self.select_callback
        self.add_item(self.select)
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(content=f"Has seleccionado **{dino_selected}**. Ahora selecciona la stat:", view=view)
...
class AlarmSelectView(discord.ui.View):
    def __init__(self, bot):
        ...
        self.select = discord.ui.Select(placeholder="Elige cuándo sonará la alarma...", min_values=1, max_values=1, options=options)
        self.select.callback = self.select_callback
        self.add_item(self.select)
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        async with aiosqlite.connect(self.bot.db_name) as db:
            await db.execute("""
                ...
                VALUES (?, ?, ?)
            """, (interaction.user.id, interaction.channel_id, alert_time.strftime('%Y-%m-%d %H:%M:%S')))
            await db.commit()
            
        await interaction.response.edit_message(content=f"✅ Alarma configurada. Te avisaré en **{horas_select} horas**.", view=None)
...
class BreedingDashboardView(discord.ui.View):
    ...
    @discord.ui.button(label="Nueva muta", style=discord.ButtonStyle.primary, custom_id="breeding_nueva_muta_btn", emoji="🧬")
    async def nueva_muta_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        async with aiosqlite.connect(self.bot.db_name) as db:
            cursor = await db.execute("SELECT DISTINCT especie FROM dinos ORDER BY especie ASC")
            rows = await cursor.fetchall()
            
        if not rows:
            await interaction.response.send_message("❌ No hay dinos registrados en la base de datos.", ephemeral=True)
            return
            
        dinos = [row[0] for row in rows]
        view = DinoSelectView(self.bot, dinos)
        await interaction.response.send_message("Selecciona el Dino al que aplicarle la muta (+2):", view=view, ephemeral=True)

    @discord.ui.button(label="Alarmas", style=discord.ButtonStyle.secondary, custom_id="breeding_alarms_btn", emoji="⏰")
    async def alarmas_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        view = AlarmSelectView(self.bot)
        await interaction.response.send_message("Selecciona el tiempo de la alarma para este canal:", view=view, ephemeral=True)
...
async def update_breeding_dashboards(bot):
    ...
    # Construir Embed
    if not rows:
        embed = discord.Embed(title="🧬 Líneas de Crianza", description="No hay líneas registradas aún.", color=discord.Color.gold())
        embed.set_footer(text="💡 Usa /linea_add para registrar o /linea_mod para actualizar.")
    else:
        embed = discord.Embed(title="🧬 Líneas de Crianza (Top Stats)", color=discord.Color.gold())
        for row in rows:
            ...
            stats_list = []
            if hp > 0: stats_list.append(f"❤️ HP: **{hp}**")
            if stam > 0: stats_list.append(f"⚡ Stam: **{stam}**")
            if weight > 0: stats_list.append(f"⚖️ Peso: **{weight}**")
            if melee > 0: stats_list.append(f"⚔️ Melee: **{melee}**")
            if oxy > 0: stats_list.append(f"🫧 Oxy: **{oxy}**")
            if food > 0: stats_list.append(f"🍖 Food: **{food}**")
            if speed > 0: stats_list.append(f"💨 Speed: **{speed}**")
        ...
        embed.set_footer(text="💡 Usa /linea_add para añadir nuevo dino | /linea_mod para modificar stats.")
...
class Breeding(commands.Cog):
    ...
    async def check_alarms(self):
        try:
            ...
            async with aiosqlite.connect(self.bot.db_name) as db:
                db.row_factory = aiosqlite.Row
                cursor = await db.execute("SELECT * FROM breeding_alarms WHERE alert_time <= ?", (now_str,))
                alarms = await cursor.fetchall()

                if not alarms:
                    return

                from main import DismissAlarmView # Asegurar importar la vista persistente

                for alarm in alarms:
                    try:
                        channel = self.bot.get_channel(alarm['channel_id']) or await self.bot.fetch_channel(alarm['channel_id'])
                        if channel:
                            view = DismissAlarmView()
                            await channel.send(f"🔔 <@{alarm['user_id']}> Tu alarma ha expirado.", view=view)
                    except Exception as e:
                        logging.getLogger("ArkTribeBot").error(f"Error procesando alarma {alarm['id']}: {e}")

                    # Borrar ejecutada
                    await db.execute("DELETE FROM breeding_alarms WHERE id = ?", (alarm['id'],))
                await db.commit()
        except Exception as e:
            logging.getLogger("ArkTribeBot").error(f"Error en loop de breeding alarms: {e}")
    ...
    async def upsert_stat(self, dino, stat_col, puntos):
        ...
        async with aiosqlite.connect(self.bot.db_name) as db:
            ...
            if row:
                ...
                await db.execute(f"UPDATE dinos SET {stat_col} = ? WHERE especie = ?", (puntos, dino))
                action = "stats actualizados"
                
                # Log Mutations
                if diff == 2:
                    logging.getLogger("ArkTribeBot").info(f"MUTATION: {dino} {stat_col} +2")
                elif diff == 4:
                    logging.getLogger("ArkTribeBot").info(f"DOUBLE MUTATION: {dino} {stat_col} +4")
                    
            else:
                # Insert
                await db.execute(f"INSERT INTO dinos (especie, {stat_col}) VALUES (?, ?)", (dino, puntos))
                action = "registrada (nueva línea)"
            
            await db.commit()
        return action

    @app_commands.command(name="linea_add", description="Registra/Actualiza una estadística para una línea.")
    ...
    async def linea_add(self, interaction: discord.Interaction, dino: str, estadistica: app_commands.Choice[str], puntos: int):
        action = await self.upsert_stat(dino, estadistica.value, puntos)
        
        await interaction.response.send_message(f"✅ 🧬 **{dino}** con **{puntos}** en **{estadistica.name}** {action}.", ephemeral=False)
        await update_breeding_dashboards(self.bot)
    ...
    async def dino_autocomplete(self, interaction: discord.Interaction, current: str) -> list[app_commands.Choice[str]]:
        async with aiosqlite.connect(self.bot.db_name) as db:
            cursor = await db.execute("SELECT DISTINCT especie FROM dinos WHERE especie LIKE ? ORDER BY especie ASC LIMIT 25", (f"%{current}%",))
            rows = await cursor.fetchall()
        
        return [app_commands.Choice(name=row[0], value=row[0]) for row in rows]

    @app_commands.command(name="linea_mod", description="Modifica una estadística específica (Igual que linea_add).")
    @app_commands.choices(estadistica=STAT_CHOICES)
    @app_commands.describe(
        dino="Especie del dino (ej. Rex)",
    ...
    async def linea_mod(self, interaction: discord.Interaction, dino: str, estadistica: app_commands.Choice[str], puntos: int):
        # Es funcionalmente idéntico a linea_add con la nueva lógica de 'Single Row'
        action = await self.upsert_stat(dino, estadistica.value, puntos)
        
        await interaction.response.send_message(f"✅ Estadística modificada: **{dino}** -> **{estadistica.name}**: {puntos}.", ephemeral=False)
        await update_breeding_dashboards(self.bot)
    ...
    @app_commands.command(name="lineas", description="Muestra el panel de líneas de crianza (Auto-actualizable).")
    async def lineas(self, interaction: discord.Interaction):
        ...
        async with aiosqlite.connect(self.bot.db_name) as db:
            await db.execute("INSERT INTO breeding_messages (channel_id, message_id) VALUES (?, ?)", (interaction.channel_id, message.id))
            await db.commit()
    ...
    async def linea_ver(self, interaction: discord.Interaction, dino: str):
        ...
        if not row:
            await interaction.response.send_message(f"No se encontró información para **{dino}**.", ephemeral=True)
            return
    ...
    @app_commands.command(name="log_mutas", description="Muestra las últimas 20 mutaciones registradas en el servidor.")
                        ...
                        for line in f:
                            if "MUTATION:" in line:
                                ...
                                try:
                                    ...
                                    if "DOUBLE" in line:
                                        mutations.append((timestamp, f"⏰ `{timestamp}`: **Doble muta** 🧬 **{dino}** en **{stat}**"))
                                    else:
                                        mutations.append((timestamp, f"⏰ `{timestamp}`: **Muta** 🧬 **{dino}** en **{stat}**"))
                                except:
                                    mutations.append((timestamp, f"`{timestamp}`: {content}"))

            if not mutations:
                await interaction.response.send_message("No se han registrado mutaciones históricamente.", ephemeral=True)
            else:
                ...
                embed = discord.Embed(title="🧬 Registro de Mutaciones (Últimas 20)", description=response_text, color=discord.Color.green())
