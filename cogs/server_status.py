...
class ServerStatus(commands.Cog):
    ...
    async def get_server_embed(self, server_name):
        ...
        try:
            ...
            embed = discord.Embed(title=f"⚠️ Error: {server_name}", description=f"No se pudo conectar.\n`{e}`", color=discord.Color.red())
            embed.set_footer(text="Se reintentará en 5 minutos.")
            return embed

    @app_commands.command(name="status", description="Muestra el estado de un servidor de ARK (consulta única).")
    @app_commands.describe(mapa="Selecciona el servidor/mapa a consultar")
    @app_commands.choices(mapa=SERVER_CHOICES)
    async def status(self, interaction: discord.Interaction, mapa: app_commands.Choice[str]):
        ...
        if embed:
            ...
            await interaction.followup.send("❌ Servidor no configurado.", ephemeral=True)

    @app_commands.command(name="status_permanente", description="Crea un mensaje que se actualiza cada 2 minutos con el estado.")
    ...
    async def status_permanente(self, interaction: discord.Interaction, mapa: app_commands.Choice[str]):
    ...
    async def status_loop(self):
        ...
        async with aiosqlite.connect(self.bot.db_name) as db:
            ...
            cursor = await db.execute("SELECT id, channel_id, message_id, map_name FROM status_messages")
            ...
            for row in rows:
                ...
                try:
                    ...
                    if not channel:
                        logger.warning(f"Canal {channel_id} no encontrado. Eliminando mensaje persistente {row_id}.")
                        messages_to_remove.append(row_id)
                        continue

                    message = await channel.fetch_message(message_id)
                    new_embed = await self.get_server_embed(map_name) # Reutilizamos la lógica
    ...
    async def get_global_status_embed(self):
        ...
        embed = discord.Embed(title="🌐 Estado Global de Servidores de ARK", color=discord.Color.blue())
        ...
        if empty_servers:
            empty_list_str = "\n".join([f"🔸 **{s['name']}** - `{s['ping']}ms`" for s in empty_servers])
            embed.add_field(name="🟡 Servidores Vacíos (Online)", value=empty_list_str, inline=False)
            
        # 3. Servidores Offline
        if offline_servers:
            offline_list_str = "\n".join([f"❌ **{s}**" for s in offline_servers])
            embed.add_field(name="🔴 Servidores Offline / Timeout", value=offline_list_str, inline=False)
                
        embed.description = f"**Total de jugadores en la red:** {total_players}/{total_max}"
        embed.set_footer(text="Actualizado automáticamente cada 2 minutos.")
        
        return embed

    @app_commands.command(name="status_online", description="Muestra todos los servidores, jugadores y nombres, actualizándose cada 2 mins.")
    ...
    async def global_status_loop(self):
        ...
        async with aiosqlite.connect(self.bot.db_name) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute("SELECT id, channel_id, message_id FROM status_online_messages")
            rows = await cursor.fetchall()
