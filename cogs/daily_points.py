...
class DailyPointsView(discord.ui.View):
    ...
    @discord.ui.button(label="Completado", style=discord.ButtonStyle.success, emoji="✅", custom_id="daily_points_completado_btn")
    async def completado_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        now_utc = datetime.now(ZoneInfo("UTC"))
        # Asumimos ES por simplicidad para la estampa visual, o mostramos la fecha local del receptor si la supiéramos,
        # pero para el log visual del DM con UTC ajustado sirve.
...
class DailyPoints(commands.Cog):
    ...
    @app_commands.command(name="puntos_diarios", description="Activa o desactiva las notificaciones diarias para votar los mapas.")
    ...
    async def puntos_diarios(self, interaction: discord.Interaction, estado: app_commands.Choice[str], hora: int = 8, zona: app_commands.Choice[str] = None):
        ...
        async with aiosqlite.connect(self.bot.db_name) as db:
            if estado.value == "on":
                try:
                    ...
                    await interaction.response.send_message(f"✅ **Notificaciones activadas.** Te avisaré todos los días a las **{hora:02d}:00** (Hora de {zona_nombre}) para votar.", ephemeral=True)
                except Exception as e:
                    await interaction.response.send_message(f"❌ Error al activar: {e}", ephemeral=True)
            else:
                try:
                    await db.execute("DELETE FROM daily_points_users WHERE user_id = ?", (user_id,))
                    await db.commit()
                    await interaction.response.send_message("🔕 **Notificaciones desactivadas.** Ya no te enviaré mensajes diarios.", ephemeral=True)
                except Exception as e:
    ...
    async def points_loop(self):
        ...
        async with aiosqlite.connect(self.bot.db_name) as db:
            ...
            cursor = await db.execute("SELECT user_id, alert_hour, timezone, last_sent_date FROM daily_points_users")
            ...
            for u in users:
                try:
                    ...
                    # Verificamos si estamos dentro de la hora objetivo
                    if user_time.hour == target_hour:
                        if u['last_sent_date'] != current_date_str:
                            users_to_notify.append((u['user_id'], current_date_str))
                except Exception as e:
                    logger.error(f"[DailyPoints] Error calculando hora para usuario {u['user_id']}: {e}")
            ...
            if users_to_notify:
                ...
                for uid, date_str in users_to_notify:
                    try:
                        ...
                        # Actualizar en BD que ya se envió hoy a este usuario
                        await db.execute("UPDATE daily_points_users SET last_sent_date = ? WHERE user_id = ?", (date_str, uid))
                    except discord.Forbidden:
                        logger.warning(f"[DailyPoints] No pude enviar DM a {uid}. Posiblemente tiene DMs cerrados.")
                    except Exception as e:
                        logger.error(f"[DailyPoints] Error enviando recordatorio a {uid}: {e}")
