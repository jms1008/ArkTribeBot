...
BASE_MAPS = ["The Island", "Scorched Earth", "Aberration", "Extinction", "Genesis 1", "Genesis 2", "The Center", "Ragnarok", "Valguero", "Crystal Isles", "Lost Island", "Fjordur"]
...
async def update_blacklist_dashboards(bot):
    ...
    if not rows:
        embed = discord.Embed(title="☠️ Blacklist de Tribu", description="No hay jugadores en la lista negra.", color=discord.Color.darker_grey())
        embed.set_footer(text="💡 /blacklist_add para añadir | /blacklist_mod para modificar")
    else:
        embed = discord.Embed(title="☠️ Blacklist de Tribu", color=discord.Color.dark_grey())
        for row in rows:
            # ID: Player [Tribe] - Map
            # Notes
            embed.add_field(
        ...
        embed.set_footer(text="💡 /blacklist_add para añadir | /blacklist_mod para modificar | Botón para borrar.")
...
async def update_kda_dashboards(bot):
    ...
    if not rows:
        embed = discord.Embed(title="☠️ Ranking de El Más Manco", description="Todavía no hay bajas registradas en la tribu.", color=discord.Color.dark_red())
        ...
        embed.set_footer(text="💡 El Bot se actualiza en vivo leyendo los logs del servidor.")
...
class BlacklistView(discord.ui.View):
    ...
    @discord.ui.button(label="Eliminar (por ID)", style=discord.ButtonStyle.danger, custom_id="blacklist_delete_btn", emoji="🗑️")
...
class Warfare(commands.Cog):
    ...
    # --- Autocompletes ---
    async def tribe_autocomplete(self, interaction: discord.Interaction, current: str) -> list[app_commands.Choice[str]]:
        async with aiosqlite.connect(self.bot.db_name) as db:
            cursor = await db.execute("SELECT DISTINCT tribe FROM blacklist WHERE tribe LIKE ? ORDER BY tribe ASC LIMIT 25", (f"%{current}%",))
            rows = await cursor.fetchall()
    ...
    @app_commands.command(name="blacklist", description="Muestra el dashboard de la Blacklist (Auto-actualizable).")
    async def blacklist(self, interaction: discord.Interaction):
        ...
        async with aiosqlite.connect(self.bot.db_name) as db:
            await db.execute("INSERT INTO blacklist_messages (channel_id, message_id) VALUES (?, ?)", (interaction.channel_id, message.id))
            await db.commit()
            
        await update_blacklist_dashboards(self.bot)

    @app_commands.command(name="blacklist_add", description="Añade un jugador a la Blacklist.")
    ...
    async def blacklist_add(self, interaction: discord.Interaction, jugador: str, tribu: str, mapa: str, notas: str):
        ...
        await interaction.response.send_message(f"✅ **{jugador}** ({tribu}) añadido a la blacklist.", ephemeral=False)
        await update_blacklist_dashboards(self.bot)
    ...
    @app_commands.command(name="blacklist_mod", description="Modifica una entrada de la Blacklist.")
    @app_commands.describe(id="ID de la entrada", campo="Campo a modificar", nuevo_valor="Nuevo valor")
    ...
    async def blacklist_mod(self, interaction: discord.Interaction, id: int, campo: app_commands.Choice[str], nuevo_valor: str):
        ...
        async with aiosqlite.connect(self.bot.db_name) as db:
            ...
            if not row:
                await interaction.response.send_message(f"❌ No existe la entrada ID {id}.", ephemeral=True)
                return
            
            # Update
            col_name = campo.value
            await db.execute(f"UPDATE blacklist SET {col_name} = ? WHERE id = ?", (nuevo_valor, id))
            await db.commit()

        await interaction.response.send_message(f"✅ ID {id} actualizado: **{campo.name}** -> {nuevo_valor}", ephemeral=False)
        await update_blacklist_dashboards(self.bot)
    ...
    async def sos(self, interaction: discord.Interaction, tipo: app_commands.Choice[str] = None, mapa: str = None, atacantes: int = None, defensores: int = None, notas: str = None):
        ...
        if not tipo and not mapa and not atacantes and not defensores and not notas:
            ...
            if mapa: embed.add_field(name="🗺️ Mapa", value=f"**{mapa}**", inline=True)
            if tipo: embed.add_field(name="🔥 Tipo", value=tipo.value, inline=True)
            
            # Fila de números
            atack_str = str(atacantes) if atacantes is not None else "?"
            def_str = str(defensores) if defensores is not None else "?"
            embed.add_field(name="⚔️ Status", value=f"👿 **Enemigos:** {atack_str}\n🛡️ **Aliados:** {def_str}", inline=False)
            
            if notas: embed.add_field(name="📝 Notas", value=notas, inline=False)
    ...
    @app_commands.command(name="ranking", description="Muestra el panel en vivo del Ranking K/D/A (El Más Manco).")
    async def ranking(self, interaction: discord.Interaction):
        ...
        async with aiosqlite.connect(self.bot.db_name) as db:
            await db.execute("INSERT INTO kda_messages (channel_id, message_id) VALUES (?, ?)", (interaction.channel_id, message.id))
            await db.commit()
            
        await update_kda_dashboards(self.bot)
        
    @app_commands.command(name="ranking_char_add", description="Vincula un personaje In-Game a un jugador para contar sus kills/muertes.")
    @app_commands.describe(jugador="Nombre del jugador de Tribu", personaje="Nombre exacto del personaje en ARK")
    async def ranking_char_add(self, interaction: discord.Interaction, jugador: str, personaje: str):
        async with aiosqlite.connect(self.bot.db_name) as db:
            # Insertar en tabla de personajes (ignora si existe, o reemplaza)
            await db.execute("INSERT INTO tribe_characters (character_name, player_name) VALUES (?, ?) ON CONFLICT(character_name) DO UPDATE SET player_name=excluded.player_name", (personaje, jugador))
            # Asegurarse que el jugador existe en la tabla principal KDA aunque empiece a 0
            await db.execute("INSERT OR IGNORE INTO tribe_kda (player_name, kills, deaths) VALUES (?, 0, 0)", (jugador,))
            await db.commit()
            
        await interaction.response.send_message(f"✅ Ahora el personaje in-game **{personaje}** registrará muertes y bajas para el jugador **{jugador}**.", ephemeral=False)
        await update_kda_dashboards(self.bot)
        
    @app_commands.command(name="ranking_char_remove", description="Desvincula un personaje del sistema de KDA.")
    @app_commands.describe(personaje="Nombre exacto del personaje en ARK")
    async def ranking_char_remove(self, interaction: discord.Interaction, personaje: str):
        async with aiosqlite.connect(self.bot.db_name) as db:
            await db.execute("DELETE FROM tribe_characters WHERE character_name = ?", (personaje,))
            await db.commit()
            
        await interaction.response.send_message(f"🗑️ Personaje **{personaje}** eliminado del registro de logs KDA.", ephemeral=False)
        
    @app_commands.command(name="ranking_remove", description="¡ADMIN! Borra a un jugador entero del KDA Tracker.")
    ...
    async def ranking_remove(self, interaction: discord.Interaction, jugador: str):
        async with aiosqlite.connect(self.bot.db_name) as db:
            ...
            await db.execute("DELETE FROM tribe_characters WHERE player_name = ?", (jugador,))
            await db.commit()
            
        await interaction.response.send_message(f"🗑️ El jugador **{jugador}** ha sido borrado del Leaderboard (Kills y Muertes reseteadas).", ephemeral=False)
        await update_kda_dashboards(self.bot)
