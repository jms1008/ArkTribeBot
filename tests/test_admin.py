...
async def test_wipe_db_command(admin_cog, mock_interaction, mock_bot, mocker):
    ...
    await mock_bot.db.execute(
        "INSERT INTO blacklist (guild_id, player) VALUES (?, ?)", (guild_id, "Enemy")
    )
    await mock_bot.db.execute(
        "INSERT INTO blacklist (guild_id, player) VALUES (?, ?)", (guild_id + 1, "Otro")
    )
    await mock_bot.db.execute(
        "INSERT INTO scouts (guild_id, tribu_enemiga) VALUES (?, ?)", (guild_id, "X")
    )
    ...
    remaining_bl = await mock_bot.db.fetchall(
        "SELECT id FROM blacklist WHERE guild_id = ?", (guild_id,)
    )
    remaining_scouts = await mock_bot.db.fetchall(
        "SELECT id FROM scouts WHERE guild_id = ?", (guild_id,)
    )
    assert remaining_bl == []
    assert remaining_scouts == []

    # Los datos de OTRO guild deben seguir intactos.
    other_bl = await mock_bot.db.fetchall(
        "SELECT player FROM blacklist WHERE guild_id = ?", (guild_id + 1,)
    )
