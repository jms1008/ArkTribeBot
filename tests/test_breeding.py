...
async def test_upsert_stat_updates_existing(breeding_cog, mock_bot):
    ...
    row = await mock_bot.db.fetchone("SELECT hp FROM dinos WHERE especie = ? AND guild_id = ?", ("Rex", 1))
    assert row["hp"] == 52
...
async def test_upsert_stat_isolates_by_guild(breeding_cog, mock_bot):
    ...
    row_g1 = await mock_bot.db.fetchone("SELECT hp FROM dinos WHERE especie = ? AND guild_id = ?", ("Rex", 1))
    row_g2 = await mock_bot.db.fetchone("SELECT hp FROM dinos WHERE especie = ? AND guild_id = ?", ("Rex", 2))
    assert row_g1["hp"] == 50
    assert row_g2["hp"] == 80
...
async def test_breeding_alarm_can_be_inserted(breeding_cog, mock_bot):
    ...
    await mock_bot.db.execute(
        "INSERT INTO breeding_alarms (guild_id, user_id, channel_id, alert_time) VALUES (?, ?, ?, ?)",
        (1, 42, 100, "2026-01-01 12:00:00"),
    )
    await mock_bot.db.commit()

    row = await mock_bot.db.fetchone("SELECT user_id FROM breeding_alarms WHERE guild_id = ?", (1,))
    assert row is not None
