...
async def run_test():
    ...
    async with aiosqlite.connect(bot.db_name) as db:
        ...
        await db.execute("INSERT INTO k4ultra_sessions (player_name, map_name, start_time, end_time, is_active) VALUES (?, ?, ?, ?, 0)", ("123", "Isla", thirty_mins_ago, thirty_mins_ago))
        await db.execute("INSERT INTO k4ultra_sessions (player_name, map_name, start_time, end_time, is_active) VALUES (?, ?, ?, ?, 0)", ("123_1", "Aberration", one_min_ago, one_min_ago))
        await db.execute("INSERT INTO k4ultra_sessions (player_name, map_name, start_time, end_time, is_active) VALUES (?, ?, ?, ?, 0)", ("123_2", "Gen2", thirty_mins_ago, thirty_mins_ago))
    ...
    async def gather_player_data(all_fetched):
        ...
        async with aiosqlite.connect(bot.db_name) as db:
            ...
            cursor = await db.execute("SELECT id, player_name, map_name FROM k4ultra_sessions WHERE is_active = 1")
            print("ACTIVE SESSIONS:", [dict(r) for r in await cursor.fetchall()])

    print("------ TICK 1 ------")
    await gather_player_data([
        {"map": "Gen2", "raw_name": "123"},
        {"map": "Aberration", "raw_name": "123"}
    ])
